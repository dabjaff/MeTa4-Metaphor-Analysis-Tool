...
def apply_mipvu_filters(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series, pd.Series]:
    ...
    is_vuamc = str(df.attrs.get('corpus', '')).lower() == 'vuamc'
    
    if is_vuamc:
        # VUAMC-specific filters (hardcoded as before)
        keep = ~df['subtype'].str.upper().isin({'DFMA', 'DFMA_PUNCT'}) & (df['metaphor_function'].str.upper() != 'DFMA')
    else:
        # User file: No DFMA filtering; keep all rows unless metaphor_function is empty/NaN
        keep = df['metaphor_function'].astype(str).str.len() > 0
    ...
    if is_vuamc:
        # VUAMC-specific MRW logic (hardcoded as before)
        typ = df_f['type']
        ...
        # User file: Flexible MRW based on any non-empty metaphor_function
        mrw_mask = df_f['metaphor_function'].astype(str).str.len() > 0
